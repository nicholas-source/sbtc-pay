...
;;
;; SECURITY FIXES from v4 audit:
;; - [CRITICAL] Admin-suspended: new `admin-suspended` field prevents self-reactivation after admin suspension
;; - [CRITICAL] Cancel-invoice: blocks cancellation when partial payments exist (must refund first)
;; - [HIGH] Close-expired-invoice: anyone can mark expired invoices on-chain
;; - [HIGH] Symmetric fee cap: fee changes limited both up AND down by MAX_FEE_BPS_CHANGE
;; - [HIGH] Reactivate merchant: works even when contract is paused (consistent with deactivate)
...
;; - [MEDIUM] Merchant-updated event: now includes field data
;;
;; PRIOR FIXES (from v3 -> v4):
...
;; - [MEDIUM] Resume subscription: respect original interval
;; - [MEDIUM] Deactivated merchants can't receive payments
;; - [MEDIUM] Refund window: 30 days (4320 blocks)
...
;; [v5] Cancel blocked - must refund partial payments first
(define-constant ERR_CANCEL_HAS_PAYMENTS (err u3009))
;; [v5] Invoice not expired yet
...
;; Merchant registry
;; [v5] Added admin-suspended field to prevent self-reactivation after admin action
...
;; [v5] Added first-payment-at for accurate refund window tracking on partial payments
(define-map invoices
  uint
  {
    merchant: principal,
...
;; Calculate platform fee (minimum 1 sat if amount > 0)
...
;; [v5] Absolute difference for symmetric fee cap
(define-private (abs-diff (a uint) (b uint))
  (if (>= a b) (- a b) (- b a))
)

;; Safe fee transfer: skips if amount is 0 or sender == recipient (self-transfer guard)
(define-private (safe-fee-transfer (fee uint) (sender principal))
  (begin
...
;; [v5] Event now includes field data
...
;; [v5 FIX] No operational check (consistent with deactivate - merchant should always be able to restore)
;; [v5 FIX] Blocked when admin-suspended - must be unsuspended by admin first
(define-public (reactivate-merchant)
  (let (
    (caller tx-sender)
    (merchant (unwrap! (map-get? merchants caller) ERR_MERCHANT_NOT_FOUND))
  )
    ;; [v5] Cannot self-reactivate if admin-suspended
...
;; [v5 FIX] Enforces MAX_EXPIRY_BLOCKS on new expiry
(define-public (update-invoice
  ...
  (let (
    ...
    ;; [v5 FIX] Enforce max expiry on update
...
;; Cancel invoice
;; [v5 FIX] Cannot cancel invoices with partial payments - must refund first
(define-public (cancel-invoice (invoice-id uint))
  (let (
    ...
    ;; [v5 FIX] Block cancellation if payments received - merchant must refund first
...
;; [v5 NEW] Close expired invoice - anyone can call to update on-chain status
;; This enables on-chain expiration enforcement without relying solely on off-chain checks
(define-public (close-expired-invoice (invoice-id uint))
  (let (
    (invoice (unwrap! (map-get? invoices invoice-id) ERR_INVOICE_NOT_FOUND))
  )
    ;; Must be pending or partial
    (asserts! (or 
      (is-eq (get status invoice) STATUS_PENDING)
      (is-eq (get status invoice) STATUS_PARTIAL)
...
;; SECURITY: Uses Check-Effects-Interactions (CEI) pattern
;; SECURITY: Self-transfer guard via safe-fee-transfer
(define-public (pay-invoice (invoice-id uint) (amount uint))
  (let (
    ...
    ;; Check merchant is still active
    (asserts! (get is-active merchant) ERR_MERCHANT_INACTIVE)
    
    ;; [v5] Check merchant not admin-suspended
    (asserts! (not (get admin-suspended merchant)) ERR_MERCHANT_SUSPENDED)
    ...
    (let (
      ...
      ;; Transfer fee (safe: skips if caller == fee-recipient)
      (try! (safe-fee-transfer fee caller))
...
;; SECURITY: CEI pattern + self-transfer guard
...
;; [v5 FIX] Refund window uses first-payment-at instead of paid-at for partial payment coverage
(define-public (refund-invoice (invoice-id uint) (refund-amount uint) (reason (string-utf8 256)))
  (let (
    ...
    ;; [v5 FIX] Enforce refund window from first payment (not paid-at which is only set on full payment)
...
;; Resume subscription
;; SECURITY FIX: respects original interval instead of resetting to now
(define-public (resume-subscription (subscription-id uint))
  (let (
    (caller tx-sender)
    (sub (unwrap! (map-get? subscriptions subscription-id) ERR_SUBSCRIPTION_NOT_FOUND))
    ;; Calculate proper next-payment-at based on last payment + interval
    (proper-next (+ (get last-payment-at sub) (get interval-blocks sub)))
    ;; If the proper next is in the past, the payment is already due - set to now
...
;; Update platform fee (max 5%, max 1% change per update in either direction)
;; [v5 FIX] Symmetric cap - prevents both rapid increase AND rapid reduction
(define-public (set-platform-fee (new-fee-bps uint))
  (let (
    ...
    ;; [v5 FIX] Cap change magnitude in either direction
...
;; [v5 FIX] Sets admin-suspended flag so merchant cannot self-reactivate
...
;; [v5 NEW] Unsuspend merchant (owner only)
;; Clears admin-suspended flag. Merchant must still call reactivate-merchant to resume operations.
(define-public (unsuspend-merchant (merchant-addr principal))
  (let (
    (merchant (unwrap! (map-get? merchants merchant-addr) ERR_MERCHANT_NOT_FOUND))
  )
    (try! (check-is-owner))
    (asserts! (get admin-suspended merchant) ERR_MERCHANT_INACTIVE) ;; only unsuspend if actually suspended
